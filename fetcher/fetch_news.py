...
NON_US_NEGATIVE = ["india equity", "indian stock", "indonesia", "indonesian", "pakistan", "vietnam", "philippines", "thailand"]

def us_score(item: dict) -> int:
    text = (item["title"] + " " + item["raw_summary"]).lower()
    score = sum(1 for kw in US_KEYWORDS if kw in text)
    score -= sum(2 for kw in NON_US_NEGATIVE if kw in text)
    return score
...
def fetch_items() -> list[dict]:
    ...
    for source, url in US_FEEDS:
        try:
            ...
            log(f"  [err]  {source}: {ex}")
    for item in pool:
        item["_us"] = us_score(item)
    pool = [x for x in pool if x["_us"] >= 0]
    pool.sort(key=lambda x: (x["_us"], x["published_at"]), reverse=True)
...
- category: 必须严格从这 8 类里选一个（不能新造、不能多选）：大盘走势 | 地缘政治 | 金融 | 经济数据 | 财报 | 政策 | 能源 | 科技
